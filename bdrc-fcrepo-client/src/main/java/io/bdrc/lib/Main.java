...
public class Main {
    ...
    public static void main(String[] args) throws URISyntaxException, BdrcRepoException {
        ...
        BdrcRepoClient nTriplesClient = new BdrcRepoClient(fcRestEndpoint, "application/n-triples");
        BdrcRepoClient ld_jsonClient = new BdrcRepoClient(fcRestEndpoint, "application/ld+json");
        BdrcRepoClient ttlClient = new BdrcRepoClient(fcRestEndpoint, "text/turtle");

        // BdrcRepoClient nTriplesClient = new BdrcRepoClient(fcRestEndpoint, "text/turtle");
...
//        String whatapp = nTriplesClient.GetResource("Volumes");
//        _logger.debug("whatapp {}", whatapp);

// Scenario 2 - get the root resources
//        String whatapp2 = nTriplesClient.GetResource("");
...
//        ArchivalGroup ag = ArchivalGroupFactory.get(nTriplesClient,"Volumes");
//        CreateVolumes(nTriplesClient, new ArchivalGroup("Volumes", null, null), new String[]{"Volume1", "Volume2",
//                "Volume3"});


// Scenario 5: Get the volumes by name - this requires  a search on dc.title, or some other way
// to add a resource by name
        _logger.info("---------------   Triples --------------------"   );
        String Vols = nTriplesClient.GetResource("Volumes");
        _logger.info("---------------   LD json --------------------"   );
        String jsonAG = ld_jsonClient.GetResource("Volumes");
        _logger.info("---------------   text ttl --------------------"   );
        String ttlV = ttlClient.GetResource("Volumes");
//        String VolsArchiveGroup = nTriplesClient.GetResource("Volumes");
//        _logger.info(VolsArchiveGroup);
//        for (int i = 1 ; i <= 3; i++) {
//            String volumei = nTriplesClient.GetResource(String.format("Volumes/Volume%d", i ));
...
//        }
        ...
        // TxnTest(nTriplesClient, "Volumes/Volume1", "/Users/jimk/dev/tmp/Projects/fcrepo-sandbox/sample-images"
        // TxnTest(nTriplesClient, "Volumes/Volume2", "/Users/jimk/dev/tmp/Projects/fcrepo-sandbox/sample-images2");


        // Add an unnamed container to the Volumes archival group
        // nTriplesClient.AddContainer("Volumes", LdpContainer.DirectContainer,null,"W12345-I54321");

        // Add an unnamed container to the basic container:
//        String new_container = nTriplesClient.AddContainer(null, LdpContainer.DirectContainer, "urn:bdrc:arc:bdr:W1345:v1" +
//                ":W12345-I54321", null);
...
//        AddImages(new_container, nTriplesClient, source_dir);
        ...
        //AddImages("Volumes/Volume3",nTriplesClient, Paths.get("/Users/jimk/dev/tmp/Projects/fcrepo-sandbox/sample-images3"));

        // URI versionURI = nTriplesClient.AddVersion("Volumes/Volume3/", "Your Message Here - Slug");
        ...
        //      Add the root volume
//        nTriplesClient.AddContainerSlug(null, LdpContainer.DirectContainer,NagRoot,
//                "Programmatically created");
//        AddVolumes(nTriplesClient,NagRoot, NagVolumes);
//        String curVolume = String.format("%s/%s", NagRoot, NagVolumes[0]);
//
//        AddImages(curVolume, nTriplesClient, Paths.get("/Users/jimk/dev/tmp/Projects" +
//                "/fcrepo-sandbox/sample-images3"));
//        URI versionURI = nTriplesClient.AddVersion(String.format("%s/%s", NagRoot, NagVolumes[2]), "Your Message Here -
//        Slug");

        // Scenario 7. Walk the tree of Volumes
